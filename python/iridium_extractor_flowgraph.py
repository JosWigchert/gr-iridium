...
class FlowGraph(gr.top_block):
    def __init__(
        ...
        if (self._input_sample_rate / self._decimation) % self._burst_sample_rate != 0:
            raise RuntimeError(
                "Selected sample rate and decimation can not be matched. Please try a different combination. Sample rate divided by decimation must be a multiple of %d."
                % self._burst_sample_rate
            )

        self._fft_size = 2 ** round(
            math.log(self._input_sample_rate / 1000, 2)
        )  # FFT is approx. 1 ms long
        ...
        if self._decimation > 1:
            ...
            if self._decimation >= 8:
                ...
                channelizer_output_sample_rate = int(
                    round(
                        float(self._input_sample_rate)
                        / self._channels
                        * self._channelizer_over_sample_ratio
                    )
                )
                self._channel_sample_rate = channelizer_output_sample_rate
                assert (
                    channelizer_output_sample_rate
                    == self._input_sample_rate / self._decimation
                )
                ...
                self._fir_bw = (
                    self._input_sample_rate / self._channels + self._burst_width
                ) / 2
                ...
                if self._fir_tw < 0:
                    raise RuntimeError(
                        "PFB over sampling ratio not enough to create a working FIR filter. Please try a different decimation."
                    )
                ...
                if len(self._pfb_fir_filter) > 300:
                    ...
                    print(
                        "Consider reducing the decimation factor or use a decimation >= 8.",
                        file=sys.stderr,
                    )

                pfb_input_delay = (
                    len(self._pfb_fir_filter) + 1
                ) // 2 - self._channels / self._channelizer_over_sample_ratio
        ...
        if config["source"] == "osmosdr":
            ...
            for key, value in d.items():
                if key.endswith("_gain"):
                    ...
                    if gain_name is not None:
                        ...
                        print(
                            "Supported gains:", source.get_gain_names(), file=sys.stderr
                        )
            ...
            try:
                ...
                raise ImportError(
                    "gr-soapy not found. Make sure you are running GNURadio >= 3.9.2.0"
                )

            if "driver" not in d:
                print("No driver specified for soapy", file=sys.stderr)
                print(
                    "Run 'SoapySDRUtil -i' to see available drivers(factories)",
                    file=sys.stderr,
                )
                exit(1)
            dev = "driver=" + d["driver"]
            ...
            other_settings = (
                sanitize(d["other_settings"]) if "other_settings" in d else ""
            )

            # We only support a single channel. Apply tune_args and other_settings to that channel.
            source = soapy.source(
                dev, "fc32", 1, dev_args, stream_args, [tune_args], [other_settings]
            )
                ...
                if key.endswith("_gain"):
                    ...
                    if gain_name is not None:
                        ...
                        print(
                            gain_name,
                            "Gain:",
                            source.get_gain(0, gain_name),
                            "(Requested %d)" % gain_value,
                            source.get_gain_range(0, gain_name),
                            file=sys.stderr,
                        )
                    else:
                        print(
                ...
                while True:
                    try:
                        if d["time_source"] == "jacksonlabs":
                            ...
                            gps_locked = source.get_mboard_sensor(
                                "gps_locked", 0
                            ).to_bool()
            ...
            if time_source:
                    ...
                    while True:
                        try:
                            gps_time = uhd.time_spec_t(
                                source.get_mboard_sensor("gps_time").to_int()
                            )
                            break
                        except ValueError as e:
                            print(e, file=sys.stderr)
                ...
                print(
                    "USRP  time:",
                    source.get_time_last_pps(0).get_real_secs(),
                    file=sys.stderr,
                )
            else:
            ...
            if config["source"] == "stdin":
                file_source = blocks.file_descriptor_source(
                    itemsize=itemsize, fd=0, repeat=False
                )
            elif config["source"] == "object":
                from iridium.file_object_source import file_object_source

                file_source = file_object_source(
                    fileobject=config["object"], itemtype=itemtype
                )
            else:
                file_source = blocks.file_source(
                    itemsize=itemsize, filename=config["file"], repeat=False
                )
        ...
        print("Staring custom version 2.0 of Iridium Extractor")
        ...
        input_filter = gnuradio.filter.firdes.low_pass_2(
            1, self._channel_sample_rate, self._burst_width / 2, self._burst_width, 40
        )
        ...
        if raw_capture_filename:
            ...
            raw_sink = blocks.file_sink(
                itemsize=gr.sizeof_short, filename=raw_capture_filename + ".sigmf-data"
            )
        ...
        if self._use_channelizer:
            ...
            for channel in range(self._channels):
                if not self._use_fft_channelizer:
                    center = (
                        channel
                        if channel <= self._channels / 2
                        else (channel - self._channels)
                    )
                    relative_center = center / float(self._channels)
                    relative_span = 1.0 / self._channels
                    relative_sample_rate = (
                        relative_span * self._channelizer_over_sample_ratio
                    )
            ...
            #     blocks.file_sink(
            #         itemsize=gr.sizeof_gr_complex,
            #         filename="/tmp/signals/channel-%d.f32" % i,
            #     )
            ...
            if self._use_fft_channelizer:
                ...
                self._channelizer = iridium.fft_channelizer(
                    1024,
                    self._channels - 1,
                    activate_streams,
                    self._n_burst_downmixers,
                    self._max_burst_len,
                    self._max_queue_len * self._n_burst_downmixers,
                    not self._offline,
                )
            else:
                self._channelizer = gnuradio.filter.pfb.channelizer_ccf(
                    numchans=self._channels,
                    taps=self._pfb_fir_filter,
                    oversample_rate=self._channelizer_over_sample_ratio,
                )
            ...
            for i in range(self._n_burst_downmixers):
                if self._burst_to_pdu_converters:
                    ...
                    tb.msg_connect(
                        (self._burst_to_pdu_converters[i], "cpdus"),
                    ...
                    tb.msg_connect(
                        (self._channelizer, "cpdus%d" % i),
                        (self._burst_downmixers[i], "cpdus"),
                    )
                    tb.msg_connect(
                        (self._burst_downmixers[i], "burst_handled"),
                        (self._channelizer, "burst_handled"),
                    )
            ...
            burst_to_pdu = iridium.tagged_burst_to_pdu(
                self._max_burst_len,
                0.0,
                1.0,
                1.0,
            ...
            tb.msg_connect(
                (burst_downmix, "burst_handled"), (burst_to_pdu, "burst_handled")
            )

            # Final connection to the demodulator. It prints the output to stdout
            tb.msg_connect(
                (burst_downmix, "cpdus"), (self._iridium_qpsk_demod, "cpdus")
            )
        ...
        tb.msg_connect(
            (self._frame_sorter, "pdus"), (self._iridium_frame_printer, "pdus")
        )
